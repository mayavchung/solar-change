...
def decompose_dR_rk_toa_core(var_pert, var_cont,f_RK, forced=True):
    ...
    ## dR due to cloud change (direct forcing)
    Dcs_lw   = dRcs_lw - dR_Tacs - dR_Tscs - dR_wvcs_lw       
    Dcs_sw   = dRcs_sw - dR_albcs - dR_wvcs_sw                # correct
    ...
    # Compute cloud feedback explicitly
    # cloud feedback is the same for rh and conventional decomposition.
    
    # Cloud radiative effect (all-sky minus clear-sky)
    dCRE_lw = dR_lw - dRcs_lw 
    dCRE_sw = dR_sw - dRcs_sw

    # cloud feedback (exclude cloud masking on the other terms)
    dR_c_lw = dCRE_lw - (dR_Ta - dR_Tacs + dR_Ts - dR_Tscs) - (dR_wv_lw - dR_wvcs_lw) - (D_lw - Dcs_lw) # air temp Ta includes LR
    dR_c_sw = dCRE_sw - (dR_wv_sw - dR_wvcs_sw) - (dR_alb - dR_albcs) - (D_sw - Dcs_sw) 
    
    # dR_c_lw  = dR_lw - D_lw - dR_Ta - dR_Ts - dR_wv_lw        # cloud feedback calculated as residual
    # dR_c_sw  = dR_sw - D_sw - dR_alb - dR_wv_sw               # cloud feedback calculated as residual
    
    dR_resid_lw  = dR_lw - D_lw - dR_Ta - dR_Ts - dR_wv_lw - dR_c_lw    # compute residual without cloud feedback for testing later   
    dR_resid_sw  = dR_sw - D_sw - dR_alb - dR_wv_sw - dR_c_sw           
    ...
    ds_write['dR_cloud_lw']= (('time','lat','lon'),dR_c_lw) # explicit cloud feedback
    ds_write['dR_cloud_sw']= (('time','lat','lon'),dR_c_sw)
    ds_write['dR_resid_lw'] = (('time','lat','lon'),dR_resid_lw) # residual accounting for cloud feedback
    ds_write['dR_resid_sw'] = (('time','lat','lon'),dR_resid_sw)
